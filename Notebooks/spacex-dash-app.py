...
def dropdown_maker(series):
    options=[]
    options.append({'label': 'All Sites', 'value': 'ALL'})
    for entry in series.unique():
        option={"label": entry, "value": entry}
        options.append(option)
    return list(options)

dropdown_launch_sites=dropdown_maker(spacex_df["Launch Site"])
...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        names=data["class"],
        title=f'Total Success Launches By Site for {entered_site}')
        return fig
